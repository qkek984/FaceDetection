        ...
        for i, image in enumerate(args["imageList"]):
            ...
            if len(rects) == 0:
                ...
                if len(rects) == 0:# not found eyes
                    cv2.imwrite('result/noEyes/' + fileName + '.jpg', image)
                    #print(fileName + "\tno")
                    resultList.append([fileName, "No eyes"])
            for rect in rects:
                ...
                if ear < args["threshold"]:
                    ...
                    #print("bye----------------:" + str(i))
                    #cv2.putText(image, "EAR: {:.2f}".format(ear), (300, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                    cv2.imwrite('result/drowsy/' + fileName + '.jpg', image)
                    #print(fileName + "\tP")
                    resultList.append([fileName, "Closed eyes"])
                # otherwise, the eye aspect ratio is not below the blink
                # threshold, so reset the counter and alarm
                else:
                    ALARM_ON = False
                    #cv2.putText(image, "EAR: {:.2f}".format(ear), (300, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    cv2.imwrite('result/good/' + fileName + '.jpg', image)
                    #print(fileName + "\tN")
                    resultList.append([fileName, "Good"])
...
if __name__=='__main__':
    ...
    drowsy = ClosedEyes(imageList, threshold=0.33)
    txt=drowsy.findClosedEyes()
